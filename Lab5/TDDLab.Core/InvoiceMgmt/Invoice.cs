    ...
    public class Invoice : ValidatedDomainObject
    {
        ...
        public sealed class ValidationRules
        {
            public static IBusinessRule<Invoice> InvoiceNumber
            {
                get
                {
                    return new BusinessRule<Invoice>(
                        MethodBase.GetCurrentMethod().Name,
                        "Invoice number should be specified",
                        invoice => invoice.InvoiceNumber.IsNotEmpty()
                    );
                }
            }
            public static IBusinessRule<Invoice> BillingAddress
            {
                get
                {
                    return new BusinessRule<Invoice>(
                        ...
                        invoice => invoice.BillToAddress != null && invoice.BillToAddress.IsValid
                    );
                }
            }
            ...
            public static IBusinessRule<Invoice> Discount
            {
                get
                {
                    return new BusinessRule<Invoice>(
                        ...
                        invoice => invoice.Discount == null || invoice.Discount.IsValid
                    );
                }
            }
            public static IBusinessRule<Invoice> Lines
            {
                get
                {
                    return new BusinessRule<Invoice>(
                        MethodBase.GetCurrentMethod().Name,
                        "Invoice lines should all be valid",
                        invoice =>
                            invoice.Lines.IsNotEmpty()
                            && invoice
                                .Lines.Where<InvoiceLine>(item => false == item.IsValid)
                                .IsEmpty()
                    );
                }
            }
